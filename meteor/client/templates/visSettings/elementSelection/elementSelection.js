/**
 * Created by josep on 04/08/2016.
 */
...
Template.elementSelection.events({
    'div.wrapper-select-relation > div > div.selectize-input > div.item click' : function(event){
        console.log(event.target.value);
    }
});

Template.elementSelection.onRendered(function(){
    ...
    selectAtomElement.selectize.on("item_add", function(value, item){
    ...
    selectRelationElement.selectize.on("item_add", function(value, item){
        item.on("click", function(){
    ...
    selectSubset.selectize.on("item_add", function(value, item){
        item.on("click", function(){
            Session.set("selectedType", value);
            $(".general-settings").slideUp();
            $(".relation-settings").slideUp();
